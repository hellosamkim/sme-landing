...
<section class="relative text-white hero bg-forest-DEFUALT">
    <div class="lg:container flex flex-col items-center justify-center md:flex md:flex-row md:min-h-[500px] relative z-10">
        <div class="container order-2 w-full py-16 md:order-1 md:w-1/2 lg:pr-10">
            <div class="content">
                <h5 class="pb-3 text-white uppercase"><a href="/advocacy/"><?= esc_html__( 'Advocacy', 'chamber' ) ?></a> / <a href="/advocacy/policy-committees/"><?= esc_html__( 'Policy Committees', 'chamber' ) ?></a> / <p class="inline opacity-50 text-gray-light"><?= get_the_title(); ?></p></h5>
                <?php
								if ( is_singular() ) :
...
<div id="post-<?php the_ID(); ?>" class="py-16">
	...
	<div class="container mt-20">
			...
			<div class="relative">
					<h2 class="pb-10 text-white"><?= esc_html__( 'Related Policy Committees', 'chamber' ) ?></h2>
							...
							if($related_posts->have_posts()) {
								while($related_posts->have_posts()) : $related_posts->the_post() ?>
									<a href="<?= get_the_permalink($related_posts->ID) ?>" class="flex flex-col col-span-12 bg-white rounded md:flex-row lg:flex-col card lg:col-span-4 hover:card-shadow">
											<img class="md:w-1/2 lg:w-full h-[16rem] object-cover rounded-t" src="<?php if (get_the_post_thumbnail_url($related_posts->ID)) { echo get_the_post_thumbnail_url($related_posts->ID); } else { echo get_template_directory_uri(). '/src/images/placeholder-image.jpeg'; } ?>" alt="">
