...
<section class="relative text-white hero bg-indigo-DEFUALT">
        ...
        <div class="container order-2 w-full py-16 md:order-1 md:w-1/2 lg:pr-10">
            <div class="content">
							<?php if (apply_filters( 'wpml_current_language', NULL ) == "fr") {
								?> <h5 class="pb-3 text-white uppercase"><a href="/fr/resources/"><?= esc_html__( 'Ressources', 'chamber' ) ?></a> / <p class="inline opacity-50 text-gray-light"><?= get_the_title(); ?></p></h5> <?php 	
								} else {
								?> <h5 class="pb-3 text-white uppercase"><a href="/resources/"><?= esc_html__( 'Resources', 'chamber' ) ?></a> / <p class="inline opacity-50 text-gray-light"><?= get_the_title(); ?></p></h5> <?php 	
								}
								?>
                <?php
								if ( is_singular() ) :
...
<div id="post-<?php the_ID(); ?>" class="py-16">
	...
	<div class="container mt-20">
			...
			<div class="relative">
					<h2 class="pb-10 text-white"><?= esc_html__( 'Related Resources', 'chamber' ) ?></h2>
			...
			<div class="flex justify-center mt-5 mr-2 lg:justify-end">
					<a href="/advocacy/campaigns-councils/" class="text-button"><i class="fa-solid fa-chevron-right"></i> <span><?= esc_html__( 'SEE ALL RESOURCES', 'chamber' ) ?></span></a>
			</div>
	</div>
</div>